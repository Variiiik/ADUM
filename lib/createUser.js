...
async function createUser(data) {
  const { username, givenName, sn, mail, password, department, title,
          manager, ou, enabled, telephoneNumber, employeeNumber, employeeID } = data;
  ...
  if (lc.MOCK_AD) {
    ...
    const nu = {
      ...
      employeeID: employeeID || ('EMP' + (4100 + lc.MOCK_USERS.length)),
      ...
      groups: process.env.LDAP_DEFAULT_GROUP ? [process.env.LDAP_DEFAULT_GROUP] : [],
