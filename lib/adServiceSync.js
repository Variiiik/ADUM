...
// Finds a group DN by CN, searching the entire directory tree.
async function _findGroupDN(groupName, extraAttrs = []) {
  const results = await lc.search(
    lc.BASE_DN,
    `(&(objectClass=group)(cn=${lc.escapeLdap(groupName)}))`,
    ['distinguishedName', ...extraAttrs]
  );
...
async function readGroupIndex(groupName) {
  ...
  try {
    const entry = await _findGroupDN(groupName, [attrName]);
    if (!entry) return null;
    const raw = entry[attrName] || entry[attrName.toLowerCase()];
...
async function writeGroupIndex(groupName, index) {
  ...
  const ldapjs = require('ldapjs');
  try {
    const entry = await _findGroupDN(groupName);
    if (!entry) { console.warn(`[adSync] writeGroupIndex: grupp "${groupName}" ei leitud AD-st`); return; }
    const dn = entry.distinguishedName || entry.distinguishedname;
    if (!dn) return;
    const client = lc.createClient();
    await _bind(client);
    await new Promise((ok, fail) => {
